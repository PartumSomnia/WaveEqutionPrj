...
\section*{Project: Solution of the initial boundary value problem with the wave equation}
  ...
  \item Consider example functions, e.g.
    ...
    {\textbf{convergence}}. A way to do this is to consider the difference
    \begin{equation}
    ...
    ($p=2$). Note, that it would be a function of $x$, \textit{i.e.,} depends on the grid position.\\
    
    The convergence order for the entire spatial domain can be evaluated 
    taking the norm as 
    \begin{equation}
    \frac{|f(x) - f^{(h)}(x)|}{|f(x) - f^{(h/2)}(x)|} = Q
    \end{equation}
    where $Q=2^p$, and $p$ being the convergence order.\\
    
    In case of oscillatory functions, it is useful to
    ...
    f^{(h/4)}(x))$ properly scaled. This second test is called {\textit
    ...
    %
  \item {\textbf{Runge-Kutta time-integration.}} There exist various
  ...
  {\textbf{Butcher table}}%~\footnote{\url{https://en.wikipedia.org/wiki/List_of_Runge%E2%80%93Kutta_methods}}: 
  \begin{equation}
...
\item \textbf{Numerical solution to the Wave equation with periodic boundary condition} Consider now the wave equation in first-order in time form, say
  ...
  a domain $\Omega=[0,1]$ with \textbf{periodic boundary conditions}. Use as
  initial data any of the functions $s(x),g(x)$ tested above.

  {\textit{Hint 1:}} The computation of the finite differencing always require
  ...
  {\textit{Hint 2}} Count how much memory do you need with a grid of $n+2$
  ...
  labelled by time. Experiment with different ways to visualize the 1D wave. Keep in mind that the result must me printed \textit{i.e.,} making a movie is a great visualisation however impossible to 
  present on a paper. Try to find a combination of 1D or/and 2D or/and 3D plots, that are compact, efficient, simple to read and deliver the important information. 
...
\item \textbf{Stability and convergence.}
  ...
  given point depends on the information in its {\textit{domain of dependency}};
...
\begin{equation}
    \label{eq:potential}
    V(x) = \frac{V_0}{\text{cosh}^2(\kappa x + \beta)} - V_0,
\end{equation}

with the particular chose of parameters: $V_0 = 0.15$, $\kappa=0.1$, $\beta=i(\pi/2)$. \\ 
Replacing the $r_*$ in the Eq. \ref{eq:rw} with $x$, and defining the potential as in Eq. $\ref{eq:potential}$ we obtain a full system. \\

Adapt the 1+1 wave equation code with open boundaries for the solution of the RW equation. Study how a Gaussian pulse is scatterred by the PT potential. 

\textit{Hint 1} First, investigate the behaviour of the potential given. Its amplitude and the position of the minima. After implementing the potential into your wave equation, experiment with different initial conditions for the wave, -- its location and amplitude. 

\item {\textbf{2+1 Wave equation}} \textit{This is a quite time-consuming task, that if we have time we will try to work out. The result is interesting and beautiful. But a proper understanding of RW with PT potential task is more important and should come first.}
