...
def rwz_potential(r,l=2,M=1.):
    ...
    oor3 = oor**3
    A  = (1.-2.*M*oor)
    Ve = - A*(lam*lamm2sq*r3 + 6.0*lamm2sq*M*r2 + 36.*lamm2*M**2*r + 72.*M**3)/(r3*(lamm2*r + 6.*M)**2) 
...
def rwz_rhs(u, (rs,V,nr,ng)):
    ...
    z[i] = - 4.*drs*s[i-1] - 10.*othree*z[i-1] + 6.*z[i-2] - 2.*z[i-3] + othree*z[i-4]
    i = 0
    #z[i] = - 20.*drs*s[i+2] - 80.*othree*z[i+2] + 40.*z[i+3] - 15.*z[i+4] + 8.*othree*z[i+5]
    z[i] = + 5.*z[i+1] - 10.*z[i+2] + 10.*z[i+3] - 5.*z[i+4] + z[i+5]
    i = N-1
    #z[i] = - 20.*drs*s[i-2] - 80.*othree*z[i-2] + 40.*z[i-3] - 15.0*z[i-4] + 8.*othree*z[i-5]
    z[i] = + 5*z[i-1] - 10.*z[i-2] + 10.*z[i-3] - 5.*z[i-4] + z[i-5]
    # u = (z,s)
    # z,t = s 
    # s,t = z,xx + V z 
...
def run(rsmin=-300, # tortoise coordinate, grid boundaries
    ...
    ng = 2
    N = nr+2*ng
    drs = (rsmax-rsmin)/float((nr-1))
    rs = -2*drs + rsmin + drs*np.arange(N)
    dt = cfl*drs
    u = np.zeros(2*N)
    print "drs=%.4e"%drs
    print "dt =%.4e"%dt
